...
public function 修正申請の詳細内容が正しく表示されている()
{
    ...
    // 実際に表示されている内容を確認
    $response->assertSee('山田太郎'); // ユーザー名
    $response->assertSee('2026-01-15'); // 対象日
    $response->assertSee('電車遅延のため'); // 申請内容
    $response->assertSee('承認待ち'); // ステータス
    $response->assertSee($request->id); // 申請ID
}
...
public function 修正申請の承認処理が正しく行われる()
{
    ...
    $request = AttendanceRequest::factory()->withTimes()->create([
        ...
        'attendance_id' => $attendance->id,
        'attendance_date' => '2026-01-15',
        'request_start_time' => '09:30:00',  // 修正後の出勤時刻
        'request_end_time' => '18:30:00',    // 修正後の退勤時刻
        'status' => '承認待ち',
        'reason' => '電車遅延のため',
    ]);
    ...
    $this->assertDatabaseHas('attendances', [
        'id' => $attendance->id,
        'start_time' => '09:30:00',  // 修正後の時刻に更新
        'end_time' => '18:30:00',    // 修正後の時刻に更新
    ]);
}
}